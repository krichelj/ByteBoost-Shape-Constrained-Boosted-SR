...
Ookami's A64FX machines are a strong fit for that search, and would also let us carefully measure how this kind of formula search performs on that hardware.
...
\begin{itemize}[nosep, leftmargin=1.4em]
 ...
 \item \textbf{Ookami (Stony Brook):} Fujitsu A64FX nodes (ARM ISA, 512-bit SVE vector units, HBM2) for the CPU-bound, highly parallel symbolic-regression search of \cref{sec:boosting}, and in particular the certification inner loop of \cref{sec:certificates}.
\end{itemize}
...
That one-sided decidability is exactly what a hard filter needs, and evaluating~\eqref{eq:interval-bb} on each GP candidate is the inner-loop hot spot, the direct target of the A64FX performance work.
...
The \emph{search} track ports the genetic-programming symbolic-regression loop of \cref{sec:boosting} to A64FX (\cref{sec:testbeds}) and tunes the certification inner loop of \cref{sec:certificates} for the ARM/SVE architecture (useful background: C/C++ or Python performance work, or an interest in ARM/HPC profiling); a self-contained slice of it is benchmarking the hard-constrained path (\cref{sec:stage-admiss}) against the soft-constrained one (\cref{sec:soft}).
