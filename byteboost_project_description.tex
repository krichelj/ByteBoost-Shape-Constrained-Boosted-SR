...
Training large models is limited by raw computing speed; Neocortex's Cerebras systems put an entire model on one giant chip, which avoids the slowdown of splitting a model across many ordinary GPUs.
We will compare those training runs with runs we have already finished on NVIDIA GPUs (\cref{sec:baselines}).
Finding the formula is a separate problem: a large search that mainly uses ordinary processors, and whose most expensive step is the mathematical check that each candidate obeys the rules (\cref{sec:certificates}).
Stony Brook's AMA27 AmpereOne cluster is a strong fit for that search: many high-core-count Arm nodes for highly parallel genetic-programming populations, on an architecture designed for scalar and imperfectly vectorized academic codes like ours, and distinct from our existing x86 CPU baselines.
...
\begin{itemize}[nosep, leftmargin=1.4em]
 \item \textbf{Neocortex (PSC):} Cerebras wafer-scale systems for the model-pretraining grid described in \cref{sec:models,sec:datasets}, where dataflow execution and wafer-scale memory should cut training wall-clock time for the larger models without the overhead of distributed training.
 \item \textbf{AMA27 (Stony Brook):} AmpereOne A192-32M nodes (Arm ISA, 192 cores and 384\,GB DDR5 per node) for the CPU-bound, highly parallel symbolic-regression search of \cref{sec:boosting}, and in particular the certification inner loop of \cref{sec:certificates}. AMA27 is the Arm testbed featured in ByteBoost~2.0; relative to our x86 EPYC baselines it supplies architectural diversity and dense core counts well matched to population-parallel genetic programming and interval-arithmetic checking (a largely scalar / imperfectly vectorized Python+JAX workload).
\end{itemize}

\subsection{Baseline hardware (already available to us)}\label{sec:baselines}
200 NVIDIA GH200 superchips on DeltaAI at NCSA, a local four-GH200 cluster, and three 256-core AMD EPYC machines, used as our existing GPU/x86 baselines.
...
The \emph{modeling} track ports and benchmarks transformer pretraining on Cerebras (\cref{sec:testbeds}) and profiles it against our GPU runs (\cref{sec:baselines}; useful background: PyTorch, or curiosity about the Cerebras SDK).
The \emph{search} track ports the genetic-programming symbolic-regression loop of \cref{sec:boosting} to AMA27 (\cref{sec:testbeds}) and profiles the certification inner loop of \cref{sec:certificates} on AmpereOne relative to our x86 EPYC baselines (useful background: C/C++ or Python performance work, or an interest in Arm/HPC profiling); a self-contained slice of it is benchmarking the hard-constrained path (\cref{sec:stage-admiss}) against the soft-constrained one (\cref{sec:soft}).
