...
\begin{enumerate}[nosep, leftmargin=2.2em, label=\textup{(A\arabic*)}]
    ...
    The direction is worth stating, because diminishing returns is classically
    drawn as a \emph{concave} curve: that picture is of a gain, whereas~$\Lhat$
    is a loss, and a gain such as $\Lhat(x_{\min},\cdot)-\Lhat$ is concave
    exactly when~$\Lhat$ is convex. Everything below certifies the convex form,
    which is why the score of \cref{sec:soft} is called
    $v_{\mathrm{conv}}$.)
