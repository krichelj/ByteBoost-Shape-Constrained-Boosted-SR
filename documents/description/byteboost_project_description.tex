...
\begin{enumerate}[nosep, leftmargin=2.2em, label=\textup{(A\arabic*)}]
    ...
    retain it as an explicit physical prior (the search of \cref{sec:soft} folds it into the floor
...
$v_{\mathrm{mono}}(g)=0$ iff $\overline{\partial F/\partial x}\leq 0$ for all $x\in\{N,D\}$, which is the monotonicity half of~\eqref{eq:interval-bb}; likewise $v_{\mathrm{conv}}=0$ is equivalent to the convexity test and $v_{\mathrm{irred}}=0$ is equivalent to the floor test of \cref{lem:ia-sound} (hence also positivity~(A5) when $L_\infty>0$).
...
Certificates~(a)--(b) hold for $g=\Lhat_j$, so by \cref{lem:ia-sound} conditions~(i)--(iv) and~(vi) hold on each certification box~$\calI_x$ (hence~(A1)--(A3),~(A5)--(A6) on that compact slice of~$\widetilde{\bbH}$), and by \cref{lem:ord} condition~(v) holds on the half-line.
