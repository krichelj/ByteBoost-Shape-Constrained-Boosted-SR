...
\codeRef{\pyDir{src/scaling}, \pyDir{src/constraints}, \pyDir{src/search}; map: \pyFile{src/README.md}}
...
(in the concrete 4D grids used here, typically width-multiplier / weight-decay style axes and learning rate).
...
\codeRef{\pyDir{src/search/baselines}, \pyDir{src/search/residuals}, \pyDir{src/search/expression}, \pyDir{src/search/boosting}, \pyDir{src/search/soft_path}, \pyDir{src/search/hard_path}}
...
positive parameter constraints and admissible by
...
\codeRef{\pyFile{src/constraints/axioms/stage_conditions.py}, \pyDir{src/constraints/certificates}}
...
\codeRef{\pyFile{src/search/boosting/algorithm.py}}
\begin{algorithm}[H]
...
\For{$j = 1, \ldots, K$}
    ...
    \State Find
           ...
           \Comment{hard filter extension; soft path uses~\eqref{eq:penalized-bb}}
...
The primary implemented path replaces a hard admissibility reject filter with per-axiom violation scores on the same candidate ensemble $F \coloneqq \Lhat^{(j-1)}+g$, folded into the \texttt{gplearn} fitness (\cref{sec:software}).
...
The \emph{search} track ports the genetic-programming symbolic-regression loop of \cref{sec:boosting,sec:soft} to AMA27 (\cref{sec:testbeds}) and profiles the DualInterval certification inner loop of \cref{sec:certificates} on AmpereOne against our AMD EPYC (x86) CPU baselines (useful background: C/C++ or Python performance work, or an interest in Arm/HPC profiling); a self-contained slice of it is benchmarking soft-penalty search against $\lambda_a=0$ and against a hard reject filter (\cref{sec:stage-admiss}).
...
\subsection{Code skeleton map}\label{sec:code-map}
The repository ships a student implementation skeleton under \pyDir{src}, grouped into five packages.
Blue links open the corresponding paths on GitHub (\texttt{main}).
Each technical subsection above also points to those files; the table summarizes the map.
...
\cref{sec:stage-admiss,sec:certificates} & \pyDir{src/constraints} & \pyFile{src/constraints/axioms/stage_conditions.py}, \pyDir{src/constraints/certificates} \\
\cref{sec:guarantee} & \pyDir{src/constraints} & \pyFile{src/constraints/guarantee/invariants.py} \\
\cref{sec:boosting}, Alg.~\ref{alg:boosting-bb} & \pyDir{src/search} & \pyDir{src/search/baselines}, \pyDir{src/search/residuals}, \pyDir{src/search/expression}, \pyDir{src/search/boosting}, \pyDir{src/search/soft_path}, \pyDir{src/search/hard_path} \\
\cref{sec:soft} & \pyDir{src/search} & \pyDir{src/search/soft_path} \\
\cref{sec:models} & \pyDir{src/modeling} & \pyFile{src/modeling/models/transformer.py} \\
