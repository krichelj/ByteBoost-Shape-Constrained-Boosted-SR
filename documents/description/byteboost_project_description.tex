...
Checking that the full ensemble $\Lhat^{(j)}$ obeys~(A1)--(A6) of \cref{sec:axioms} is a continuum statement over $\bbH$.
The next claim reduces that continuum check to conditions on the increment alone.
...
\begin{equation}\label{eq:interval-bb}
    ...
    \overline{F} < \infty .
\end{equation}
Because the enclosures are sound, \eqref{eq:interval-bb} is a \emph{sufficient} certificate that the corresponding continuum conditions hold \emph{everywhere} on~$\calI_x$, not merely at sampled points (\cref{lem:ia-sound,sec:cert-lemmas}):
...
If $V(g)=0$ at every stage then the soft path recovers the hard-path conclusion of \cref{thm:guarantee} exactly.
...
Moreover, for every finite operator set~$\calP$ there exists $\varepsilon_{\mathrm{decay}}>0$ small enough that every correction accepted by the hard certificates~(a)--(b) has $V(g)=0$, so the soft search contains the hard one of \cref{sec:stage-admiss}.
\end{proposition}
...
Soft scores act as IA proxies for~(A1)--(A4) (with positivity~(A5) folded into the irreducible-floor gap and~(A6) into finiteness of the DualInterval enclosure / smooth operator set).
...
\cref{sec:stage-admiss,sec:certificates} & \pyDir{src/constraints} & \pyFile{src/constraints/axioms/stage_conditions.py}, \pyDir{src/constraints/certificates}, \pyDir{src/search/hard_path} \\
...
Positivity $G+g>0$ on $\bbH$ is exactly~(iii).
The joint-floor inequality $G+g>L_\infty$ on $\bbH$ is exactly~(iv).
(Here $L_\infty$ is the joint floor already certified for~$G$;~(iv) keeps it for the update.)

\paragraph{(A6).}
If $G$ is $C^1$ on $[x_{\min},\infty)$ and $g$ is $C^\infty$ on $[x_{\min},x_{\max}]$ with finite boundary values~(vi), then $G+g$ is $C^1$ on the certification box.
On the half-line beyond $x_{\max}$, the same $C^1$ regularity follows once~$g$ is realized by a finite expression tree from $\{+,-,\times,\div\}\cup\{\mathrm{pow}_p\}$ (each primitive is $C^\infty$ on $(0,\infty)$), which is the setting of \cref{sec:boosting}.
The bound $(G+g)(x_{\min},\cdot)>L_\infty$ is the restriction of~(iv).
...
In particular, writing $F=G+g$ with $G=\Lhat^{(j-1)}$, conditions~(i)--(iv) and the finiteness half of~(vi) hold on the configurations covered by the product of boxes~$\calI_x$.
\end{lemma}
...
and $\underline{F}>L_\infty$ forces $F>L_\infty$ (hence also $F>0$ when $L_\infty>0$), which is~(iv) and yields~(iii) because $G>L_\infty$ already gives $g=F-G>-G$.
Boundedness of~$\overline{F}$ is the finiteness claim.
...
Certificates~(a)--(b) accept $g=\Lhat_j$, so by \cref{lem:ia-sound} conditions~(i)--(iv) and~(vi) hold on each certification box~$\calI_x$, and by \cref{lem:ord} condition~(v) holds on the half-line.
The box covers the observed grid; on $x>x_{\max}$,~(v) together with $G=\Lhat^{(j-1)}\in\calS$ implies that the correction is negligible relative to the leading excess of~$G$, so~(A1)--(A2) of~$G$ persist for large~$x$ (the derivatives of an $O(t^{c})$ term with $c<c_x^{(0)}<0$ are $o$ of those of the leading $t^{c_x^{(0)}}$ excess).
...
\paragraph{Recovery.}
$v_{\mathrm{mono}}(g)=0$ iff $\overline{\partial F/\partial x}\leq 0$ for all $x\in\{N,D\}$, which is the monotonicity half of~\eqref{eq:interval-bb}; likewise $v_{\mathrm{conv}}=0$ and $v_{\mathrm{irred}}=0$ recover the remaining interval tests of \cref{lem:ia-sound}.
...
Thus $V(g)=0$ implies certificates~(a)--(b) (with the decay margin), and \cref{thm:guarantee} applies stage by stage.
...
Taking maxima over $x\in\{N,D\}$ shows that $v_{\mathrm{mono}}$, $v_{\mathrm{conv}}$, and $v_{\mathrm{irred}}$ upper-bound the corresponding true violations.
...
Therefore every hard-accepted correction has $v_{\mathrm{decay}}=0$ as well as vanishing mono/conv/irred/leaf scores, i.e.\ $V(g)=0$, and is a zero-penalty feasible point of~\eqref{eq:penalized-bb}.
\end{proof}
...
Every summand of~$R$ is strictly positive on~$\bbH$, hence $R(\mathbf{h}) > E > 0$.
...
\paragraph{(A3).}
Set $L_\infty \coloneqq E > 0$. For every $\mathbf{h} \in \bbH$,
\[
    \Lhat_0(\mathbf{h}) - E
    \;=\;
    \frac{A}{N^{\alpha}} + \frac{B}{D^{\beta}}
    ...
    \;>\; 0,
\]
so the joint-floor inequality of~(A3) holds strictly.
...
\paragraph{(A5)--(A6).}
Positivity~(A5) is immediate from $\Lhat_0(\mathbf{h}) > E > 0$.
For~(A6): on $[N_{\min},\infty)$ the map $N \mapsto A N^{-\alpha}$ is $C^\infty$ (hence $C^1$), the remaining terms are independent of~$N$, and
$\Lhat_0(N_{\min},\cdot) = A N_{\min}^{-\alpha} + B D^{-\beta} + R(\mathbf{h})$ is finite and strictly larger than~$E$.
