...
\codeRef{\pyFile{src/systems/hpc/profiling.py}, \pyFile{src/modeling/training/trainer.py}}
...
Symbolic search uses \texttt{gplearn}~\citep{stephens2015gplearn} with a custom fitness that adds soft interval-arithmetic axiom penalties (\cref{sec:soft}); an optional PySR backend provides an unconstrained alternate engine for ablation.
...
A hard reject filter that discards inadmissible candidates during search (\cref{sec:stage-admiss}) is a workshop extension built on the same certificate primitives.
...
An already-public \texttt{colinear\_\allowbreak scaling\_\allowbreak models} collection on Hugging Face~\citep{kricheli2025colinear} supplies trained checkpoints and loss curves from an initial DeltaAI grid for the symbolic-regression track.
...
\begin{enumerate}[nosep, leftmargin=2.2em, label=\textup{(A\arabic*)}]
    ...
    $\Lhat(\mathbf{h}) > 0$ for every $\mathbf{h} \in \widetilde{\bbH}$.
    Since $L_\infty > 0$,~(A5) is implied by the joint-floor inequality in~(A3); we
    retain it as an explicit physical prior (the soft path folds it into the floor
    score~$v_{\mathrm{irred}}$).
...
enforced exactly by the hard filter and recovered by the soft path whenever $V=0$ (\cref{sec:soft,thm:guarantee}).
...
Stage~$0$ is a generalized Chinchilla law, fit by nonlinear least squares with
...
\codeRef{\pyFile{src/constraints/axioms/stage_conditions.py}, \pyDir{src/constraints/certificates}, \pyDir{src/search/hard_path}}
Checking that the full ensemble $\Lhat^{(j)}$ obeys~(A1)--(A6) of \cref{sec:axioms} is a continuum statement on~$\widetilde{\bbH}$ and along each scale half-line.
...
(The second-derivative enclosure is what makes~(A2) / stage~(ii) checkable;~(A6) asks only for~$C^2$, while the operator set of \cref{sec:boosting} in fact yields~$C^\infty$ on $(0,\infty)$.)
...
\begin{equation}\label{eq:interval-bb}
    ...
    \overline{F} < \infty .
\end{equation}
Because the enclosures are sound, \eqref{eq:interval-bb} is a \emph{sufficient} certificate that the corresponding continuum conditions hold \emph{everywhere} on~$\calI_x$, not merely at sampled points of~$\bbH$ (\cref{lem:ia-sound,sec:cert-lemmas}):
$\overline{\partial F/\partial x} \leq 0$ forces~$\partial F/\partial x \leq 0$ on~$\calI_x$ (hence~(i) on the box), and likewise for~(ii);
$\underline{F} > L_\infty > 0$ forces~(iv) on the continuum configurations covered by the product of boxes, and hence also~(iii) / positivity~(A5);
$\overline{F} < \infty$ forces the finiteness half of~(vi), while~$C^2$ (in fact~$C^\infty$) smoothness of~$g$ on~$\calI_x$ follows from the operator set and discharges~(A6) on the box.
...
In the primary soft path (\cref{sec:soft}), the same enclosures are converted to continuous violation scores rather than used as a reject gate.
...
\codeRef{\pyDir{src/search/boosting}}
Putting the pieces together, Algorithm~\ref{alg:boosting-bb} builds the ensemble of \cref{sec:boosting}.
The \emph{primary} soft path of \cref{sec:soft} selects each correction by minimizing the penalized fitness~\eqref{eq:penalized-bb}; the workshop hard-filter extension instead constrains the stage argmin by certificates~(a)--(b) of \cref{sec:stage-admiss} (written explicitly in the pseudocode below).
\begin{algorithm}[H]
...
\Require Dataset $\calD = \{(\mathbf{h}_\ell, L(\mathbf{h}_\ell))\}_{\ell=1}^{n}$,
         stages $K$
\Ensure Ensemble $\Lhat^{(K)}$ (admissible under the hard filter; soft path aims for $V=0$ at every stage)
...
\For{$j = 1, \ldots, K$}
    ...
    \State Find $\Lhat_j$ by soft search~\eqref{eq:penalized-bb}
           \textbf{or} by
           $\argmin_{g}
           ...
           \Comment{soft primary; hard filter is the workshop extension}
...
The hard path of Algorithm~\ref{alg:boosting-bb} preserves admissibility, the joint floor, and the stage-0 rates:
\begin{theorem}[Hard-path boosting guarantee]\label{thm:guarantee}
Assume $\Lhat_0$ is the admissible stage-0 law of \cref{prop:chinchilla-admiss} (so $L_\infty = E$).
If every later correction $\Lhat_j$ ($j\geq 1$) is accepted by certificates~(a)--(b) of \cref{sec:stage-admiss}, then for all $j$: $\Lhat^{(j)} \in \calS$; writing $L_\infty^{(j)}$ for the joint floor of~$\Lhat^{(j)}$, one has $L_\infty^{(j)} = E$; and the asymptotic exponents are preserved,
...
\end{theorem}
Boosting therefore improves fit \emph{without} corrupting the interpretable constants researchers actually quote (proof in \cref{sec:guarantee-proof}).

\subsection{Soft-constrained variant}\label{sec:soft}
\codeRef{\pyFile{src/search/soft_path/violations.py}, \pyFile{src/search/soft_path/fitness.py}, \pyFile{src/search/soft_path/backends.py}}
The primary implemented path uses per-axiom violation scores on the candidate ensemble $F \coloneqq \Lhat^{(j-1)}+g$, folded into the \texttt{gplearn} fitness (\cref{sec:software}), rather than a hard reject gate.
...
\begin{proposition}[Soft-path recovery and containment]\label{prop:soft}
...
discharged in practice by the $C^\infty$ operator set of \cref{sec:boosting}), then the soft path recovers the hard-path conclusion of \cref{thm:guarantee} exactly.
...
Moreover, for every finite operator set~$\calP$ there exists $\varepsilon_{\mathrm{decay}}>0$ small enough that every correction accepted by the hard certificates~(a)--(b) has $V(g)=0$ (and finite enclosures), so the soft search contains the hard one of \cref{sec:stage-admiss}.
\end{proposition}
(See \cref{sec:soft-proof} for the proof.)
The soft scores therefore yield a \emph{measured} admissibility gap rather than silent failure.
Soft scores act as IA proxies for~(A1)--(A4) (with positivity~(A5) folded into the irreducible-floor gap and~(A6) into finiteness of the DualInterval enclosure / $C^2$---in practice $C^\infty$---operator set).
Comparing soft-penalty search ($\lambda_a>0$) against an unconstrained baseline ($\lambda_a=0$), and optionally against a true hard reject filter, is one of the empirical questions the project settles.
...
The \emph{search} track ports the genetic-programming symbolic-regression loop of \cref{sec:boosting,sec:algorithm,sec:soft} to AMA27 (\cref{sec:testbeds}) and profiles the DualInterval certification inner loop of \cref{sec:certificates} on AmpereOne against our AMD EPYC (x86) CPU baselines (useful background: C/C++ or Python performance work, or an interest in Arm/HPC profiling); a self-contained slice of it is benchmarking soft-penalty search against $\lambda_a=0$ and against a hard reject filter (\cref{sec:stage-admiss}).
...
\underline{f},\; \overline{f}
  & Interval-arithmetic lower/upper enclosures of~$f$ on~$\calI_x$
    ...
    $\underline{F}>L_\infty$ certifies~(A3)/(A5) on the box;
    $\overline{F}<\infty$ with the smooth operator set certifies~(A6). \\
a,\; v_a(g),\; V(g)
...
\cref{sec:stage-admiss,sec:certificates} & \pyDir{src/constraints}, \pyDir{src/search} & \pyFile{src/constraints/axioms/stage_conditions.py}, \pyDir{src/constraints/certificates}, \pyDir{src/search/hard_path} \\
\cref{sec:algorithm}, Alg.~\ref{alg:boosting-bb} & \pyDir{src/search} & \pyDir{src/search/boosting} \\
\cref{sec:guarantee} & \pyDir{src/constraints} & \pyFile{src/constraints/guarantee/invariants.py} \\
\cref{sec:soft} & \pyDir{src/search} & \pyDir{src/search/soft_path} \\
...
\cref{sec:testbeds,sec:baselines} (AMA27/CPU) & \pyDir{src/systems} & \pyDir{src/systems/hpc} \\
...
Proofs follow the dependency order of \cref{sec:method}: stagewise equivalence, then the certificate lemmas used by the hard filter, then the hard-path theorem, then the soft-path proposition.
...
\paragraph{(A5) and joint floor~(A3).}
Positivity $G+g>0$ on $\widetilde{\bbH}$ is exactly~(iii).
...
When $L_\infty>0$,~(iv) implies~(iii).)
...
When $L_\infty>0$, the floor test also yields positivity~(A5) /~(iii).
Smoothness~(A6) on the box follows from the $C^\infty$ operator set once finiteness holds.
\end{lemma}
...
\subsection{Hard-path boosting guarantee}\label{sec:guarantee-proof}
...
Certificates~(a)--(b) accept $g=\Lhat_j$, so by \cref{lem:ia-sound} conditions~(i)--(iv) and~(vi) hold on each certification box~$\calI_x$ (hence~(A1)--(A3),~(A5)--(A6) on that compact slice of~$\widetilde{\bbH}$), and by \cref{lem:ord} condition~(v) holds on the half-line.
The box covers the observed grid; on $x>x_{\max}$,~(v) together with $G=\Lhat^{(j-1)}\in\calS$ implies that the correction is negligible relative to the leading excess of~$G$, so~(A1)--(A2) of~$G$ persist for large~$x$ (the derivatives of an $O(t^{c})$ term with $c<c_x^{(0)}<0$ are $o$ of those of the leading $t^{c_x^{(0)}}$ excess), and~$C^2$ regularity~(A6) extends by the operator set.
...
\subsection{Soft-path guarantees}\label{sec:soft-proof}
...
\paragraph{Recovery.}
$v_{\mathrm{mono}}(g)=0$ iff $\overline{\partial F/\partial x}\leq 0$ for all $x\in\{N,D\}$, which is the monotonicity half of~\eqref{eq:interval-bb}; likewise $v_{\mathrm{conv}}=0$ recovers the convexity test and $v_{\mathrm{irred}}=0$ recovers the floor test of \cref{lem:ia-sound} (hence also positivity~(A5) when $L_\infty>0$).
The remaining interval test~$\overline{F}<\infty$ is not encoded as a soft score~$v_a$: under the smooth operator set of \cref{sec:boosting}, a completed DualInterval evaluation already yields finite enclosures, so~(A6) /~(vi) holds whenever the IA layer succeeds.
...
\[
  ...
  \max\bigl(0,\overline{\partial F/\partial x}\bigr)
\]
pointwise on the box (and likewise for the convexity and floor / positivity gaps).
...
The attainable values of $\mathrm{ord}(g,x)$ for trees over a finite power set~$\calP$ form a finite subset of~$\bbR$.
Let $\Delta$ be the minimal positive gap between $c_x^{(0)}$ and any attainable exponent strictly below it (or $\Delta=1$ if no such exponent exists).
Choosing $0<\varepsilon_{\mathrm{decay}}<\Delta$ forces: every $g$ with $\mathrm{ord}(g,x)<c_x^{(0)}$ automatically satisfies $\mathrm{ord}(g,x)\leq c_x^{(0)}-\varepsilon_{\mathrm{decay}}$.
Therefore every hard-accepted correction has $v_{\mathrm{decay}}=0$ as well as vanishing mono/conv/irred/leaf scores, i.e.\ $V(g)=0$, and already satisfies~$\overline{F}<\infty$ by~\eqref{eq:interval-bb}, so it is a zero-penalty feasible point of~\eqref{eq:penalized-bb}.
\end{proof}
...
Shape axioms~(A1),~(A2),~(A4), and~(A6) are imposed only along $x \in \{N,D\}$ with the other coordinates held fixed; under that restriction $R$ is constant in~$x$.
...
\paragraph{(A5)--(A6).}
Positivity~(A5) is immediate from $\Lhat_0(\mathbf{h}) > E > 0$ on~$\widetilde{\bbH}$.
For~(A6): on $[N_{\min},\infty)$ the map $N \mapsto A N^{-\alpha}$ is $C^\infty$ (hence $C^2$), the remaining terms are independent of~$N$, and
...
Thus $\Lhat_0$ satisfies~(A1)--(A6), i.e.\ $\Lhat_0 \in \calS$.
\end{proof}
...
Whenever $A,B,E,\alpha,\beta > 0$, the same derivative and asymptotic calculations apply verbatim, so~(A1)--(A6) continue to hold with the same $L_\infty = E$ and $c_x^{(0)}$.
\end{remark*}
