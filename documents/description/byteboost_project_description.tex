...
(in the concrete 4D grid used here, weight decay and learning rate); see also \cref{sec:notation}.
...
\begin{enumerate}[nosep, leftmargin=2.2em, label=\textup{(A\arabic*)}]
    ...
    The direction is worth stating, because the textbook form of diminishing
    returns is \emph{concavity}. That convention is for an increasing
    production or utility function, whose marginal gain falls; here~$\Lhat$ is
    a decreasing \emph{loss}, so the same shape carries the opposite sign. The
    two statements agree, since a gain such as $\Lhat(x_{\min},\cdot)-\Lhat$ is
    concave exactly when~$\Lhat$ is convex. Everything below certifies the
    convex form, which is why the score of \cref{sec:soft} is called
    $v_{\mathrm{conv}}$, and~(A2) is what
    \cref{prop:chinchilla-admiss} verifies for the stage-0 law via
    $\partial^2\Lhat_0/\partial N^2=\alpha(\alpha+1)A N^{-\alpha-2}>0$.)
...
Note that~(vi) is required on the whole half-line, not merely on the certification box: the~(A5) half of \cref{prop:stage-iff} needs $\Lhat^{(j-1)}+\Lhat_j$ to stay $C^2$ wherever~(A1)--(A2) are asserted.
For a finite expression tree this amounts to no division denominator and no $\mathrm{pow}_p$ argument vanishing on $[x_{\min},\infty)$; interval arithmetic discharges it on~$\calI_x$ and the remainder is a structural side condition, as discussed in~(a) below.
\begin{proposition}[Stagewise conditions]\label{prop:stage-iff}
Let $\Lhat^{(j-1)} \in \calS$ have joint floor $L_\infty$ and stage-0 rates $c_x^{(0)}$.
If $\Lhat_j$ satisfies~\eqref{eq:stage-admiss-bb}(i)--(vi) for each $x \in \{N,D\}$, then
$\Lhat^{(j)} = \Lhat^{(j-1)} + \Lhat_j$ lies in~$\calS$ with the same joint floor
and the same asymptotic rates.
Conversely, any update with those three properties satisfies~(i)--(iv) and~(vi)
together with the tail bound
$\Lhat_j(\mathbf{h})|_{x=t} = o\bigl(t^{c_x^{(0)}}\bigr)$.
The gap between that bound and~(v) is deliberate:~(v) is the strictly stronger
form, since it excludes borderline tails such as $t^{c_x^{(0)}}/\log t$, and we adopt
it because it is what the structural test of~(b) decides exactly on a finite
tree, while only the sufficient direction is needed by \cref{thm:guarantee}.
\end{proposition}
...
Those enclosures are exactly the inputs to the soft violation scores of \cref{sec:soft}: the per-axis monotonicity gap is $\max(0,\overline{\partial F/\partial x})$ and likewise for the other gaps, with $v_{\mathrm{mono}}$ and its companions in~\eqref{eq:violations-bb} taking $\max_{x\in\{N,D\}}$ of those per-axis quantities.
The sufficient inequalities
...
$\overline{F} < \infty$ forces the finiteness half of~(vi), and, because a finite enclosure also certifies that no denominator or $\mathrm{pow}_p$ argument vanished, yields~$C^2$ (in fact~$C^\infty$) smoothness of~$g$ on~$\calI_x$, discharging~(A5) on the box.
...
\smallskip\noindent
\emph{Where the smoothness in~(A5) actually comes from.}
The primitives are smooth only away from the zeros of their arguments: $z_1/z_2$ is $C^\infty$ where $z_2 \neq 0$, and $\mathrm{pow}_p(z)=|z|^p$ is $C^\infty$ where $z \neq 0$ (at the origin it is unbounded for $p<0$ and, for the fractional $p$ that dominate a useful~$\calP$, not even $C^2$).
A finite tree~$g$ is therefore $C^\infty$ precisely on the set where every division denominator and every $\mathrm{pow}_p$ argument is nonzero; the shorthand ``the operator set is $C^\infty$ on $(0,\infty)$'' used below abbreviates that condition and is \emph{not} a consequence of the tree merely being finite.
On the box the condition is discharged for free \emph{provided the DualInterval layer is implemented to fail loudly}: an operand interval straddling~$0$ beneath~$\div$ or $\mathrm{pow}_p$ must yield a non-finite enclosure, so a completed evaluation with $\overline{F}<\infty$ certifies that no such subtree vanished anywhere on~$\calI_x$, and with it~(A5) and the finiteness half of~(vi).
Beyond~$x_{\max}$ there is no enclosure to lean on, so the tail half of~(A5) carries the side condition that no denominator or $\mathrm{pow}_p$ argument vanishes on $(x_{\max},\infty)$ either; an implementation discharges it by restricting divisors to subtrees of certified sign, or by widening~$\calI_x$ past the range over which extrapolations are actually reported.
...
Then~(v) holds with $c_{x,j} = \mathrm{ord}(g,x)$ whenever $\mathrm{ord}(g,x) < c_x^{(0)}$ for each $x \in \{N,D\}$ (\cref{lem:ord,sec:cert-lemmas}); when two sibling subtrees share the leading exponent their leading coefficients may cancel, in which case $\mathrm{ord}$ is a sound over-estimate of the true order and the test stays sufficient.
We additionally require that each scale variable $x \in \{N,D\}$ appears as a leaf of~$g$; this is a prerequisite for the exponent rather than a constraint in its own right, since otherwise $\mathrm{ord}(g,x)$ is vacuously~$0$ and~(v) could be passed by a correction that ignores that axis.
...
The remaining two scores have no counterpart in that literature: $v_{\mathrm{decay}}$ encodes the asymptotic-rate axiom~(A4), which is specific to scaling laws, through the structural exponent $\mathrm{ord}(g,x)$ of \cref{sec:stage-admiss}(b) on the half-line $x > x_{\max}$ rather than through an interval bound on a compact box, and $v_{\mathrm{leaf}}$ is the prerequisite that makes that exponent meaningful.
...
If every later correction $\Lhat_j$ ($j\geq 1$) has $V(\Lhat_j)=0$ with finite DualInterval enclosures (equivalently, satisfies certificates~(a)--(b) of \cref{sec:stage-admiss}; see \cref{prop:soft}), then for all $j$: $\Lhat^{(j)} \in \calS$; writing $L_\infty^{(j)}$ for the joint floor of~$\Lhat^{(j)}$, one has $L_\infty^{(j)} = E$; and the asymptotic exponents are preserved,
...
\underline{f},\; \overline{f}
  & Interval-arithmetic lower/upper enclosures of~$f$ on~$\calI_x$
    ...
    $\overline{F}<\infty$ certifies~(A5) on the box, since a completed finite
    evaluation also rules out a vanishing denominator or $\mathrm{pow}_p$
    argument (\cref{sec:stage-admiss}(a)). \\
a,\; v_a(g),\; V(g)
...
Proofs follow the dependency order of \cref{sec:method}: the stagewise conditions, then the certificate lemmas, then zero-penalty implies certificates, then the boosting guarantee.
Stage-0 admissibility (\cref{prop:chinchilla-admiss}) is independent of the boosting loop and is proved last.

\subsection{Stagewise conditions}\label{sec:stage-iff-proof}
...
\paragraph{(A5).}
If $G$ is $C^2$ on $[x_{\min},\infty)$ and $g$ is $C^\infty$ on $[x_{\min},\infty)$ with finite boundary values~(vi), then $G+g$ is $C^2$ on $[x_{\min},\infty)$, which is~(A5).
The two halves of that hypothesis on~$g$ are certified separately: on the box $\calI_x=[x_{\min},x_{\max}]$ by the interval layer, and beyond it structurally.
On the half-line beyond $x_{\max}$, the same $C^2$ regularity follows once~$g$ is realized by a finite expression tree from $\{+,-,\times,\div\}\cup\{\mathrm{pow}_p\}$ \emph{and} no division denominator or $\mathrm{pow}_p$ argument vanishes there, since each primitive is $C^\infty$ away from the zeros of its arguments (\cref{sec:stage-admiss}(a), ``where the smoothness in~(A5) actually comes from''); that nonvanishing side condition is what the tail half of~(vi) asks for, and it is the setting of \cref{sec:boosting}.
...
Because $G\in\calS$, the marginal floor $L^{\infty}_{x}(G)$ exists and, by~(A4), the excess satisfies
$G|_{x=t}-L^{\infty}_{x}(G) = t^{\,c_x^{(0)}+o(1)}$ with $c_x^{(0)}<0$.
(We use only this log-rate form, not the stronger ``constant times $t^{c_x^{(0)}}$'', which~(A4) does not assert.)
Condition~(v) gives $g|_{x=t}=O(t^{c_{x,j}})$ with $c_{x,j}<c_x^{(0)}$; since $t^{c_{x,j}}/t^{\,c_x^{(0)}+o(1)} = t^{\,c_{x,j}-c_x^{(0)}+o(1)} \to 0$, the correction is negligible against the excess, and in particular $g|_{x=t}=o\bigl(t^{c_x^{(0)}}\bigr)$.
Therefore
\[
...
\]
\paragraph{Converse.}
Suppose $G+g\in\calS$ with the same joint floor and the same asymptotic rate~$c_x^{(0)}$.
Items~(i)--(iv) and~(vi) are the rearrangements already used above, read in the opposite direction.
For the tail, the difference $g=(G+g)-G$ must be $o(t^{c_x^{(0)}})$ along each axis, since otherwise the leading excess exponent of $G+g$ would differ from that of~$G$.
This is weaker than~(v): $o(t^{c_x^{(0)}})$ does not supply any single exponent $c_{x,j}<c_x^{(0)}$ with $g=O(t^{c_{x,j}})$, as $g=t^{c_x^{(0)}}/\log t$ shows.
So~(i)--(vi) are sufficient, and necessary only with~(v) relaxed to the $o(\cdot)$ bound.
We work with~(v) rather than the exact necessary condition because~(v) is what $\mathrm{ord}$ decides exactly on a finite tree (\cref{lem:ord}), and because only the sufficient direction is used in \cref{thm:guarantee}.
\end{proof}
...
Smoothness~(A5) on the box follows once finiteness holds, because a completed finite evaluation certifies that no division denominator or $\mathrm{pow}_p$ argument vanished on~$\calI_x$, and the operator set is $C^\infty$ there (\cref{sec:stage-admiss}(a)).
\end{lemma}
...
Then $g(\mathbf{h})|_{x=t}=O\bigl(t^{\mathrm{ord}(g,x)}\bigr)$ as $t\to\infty$, with matching $\Theta$ asymptotics whenever no exact cancellation of leading terms occurs.
Hence~\eqref{eq:stage-admiss-bb}(v) holds with $c_{x,j}=\mathrm{ord}(g,x)$ whenever $\mathrm{ord}(g,x)<c_x^{(0)}$, and the test is exact on trees free of leading-term cancellation.
\end{lemma}
...
The remaining interval test~$\overline{F}<\infty$ is not encoded as a soft score~$v_a$: a DualInterval evaluation that completes with finite enclosures has in particular kept every division denominator and every $\mathrm{pow}_p$ argument away from~$0$ on the box, which is exactly the condition under which the operator set of \cref{sec:boosting} is $C^\infty$ there, so~(A5) /~(vi) holds on~$\calI_x$ whenever the IA layer succeeds (\cref{sec:stage-admiss}(a)).
