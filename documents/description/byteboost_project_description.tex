...
% Pointer from a description section to the student implementation skeleton.
% Use \path so long repo paths can break at ``/'' without overfull boxes.
\newcommand{\codeRef}[1]{%
  \par\nobreak\noindent
  {\footnotesize\textit{Code skeleton:}~\path{#1}}\par\smallskip}
...
\codeRef{src/modeling/training/; src/systems/hpc/; src/search/}
...
\codeRef{src/systems/hpc/profiling.py; src/modeling/training/trainer.py}
...
\codeRef{src/modeling/training/; src/search/; src/constraints/certificates/}
...
\codeRef{src/modeling/models/config.py; src/modeling/models/llama.py}
...
\codeRef{src/scaling/data/scaling_dataset.py; src/scaling/data/corpora.py}
...
\codeRef{src/ packages scaling/, constraints/, search/ (full map in src/README.md)}

\subsection{Setup}\label{sec:setup}
\codeRef{src/scaling/setup/configuration.py; src/scaling/setup/constants.py}
...
\codeRef{src/constraints/axioms/admissibility.py}
...
\codeRef{src/search/ (baselines/, residuals/, expression/, boosting/, hard_path/)}
...
\codeRef{src/constraints/axioms/stage_conditions.py; src/constraints/certificates/}
...
\codeRef{src/search/boosting/algorithm.py}
\begin{algorithm}[H]
...
\codeRef{src/constraints/guarantee/invariants.py}
...
\codeRef{src/modeling/ (modeling track); src/search/ + src/systems/hpc/ (search track)}
...
\codeRef{src/systems/pipeline/run_discovery.py}
...
The repository ships a student implementation skeleton under \texttt{src/}, grouped into five packages.
Each technical subsection above points to the corresponding paths; the table summarizes the map.
...
\bottomrule
\endfoot
\cref{sec:setup} & \texttt{scaling/} & \texttt{setup/configuration.py}, \texttt{setup/constants.py} \\
\cref{sec:datasets} & \texttt{scaling/} & \texttt{data/scaling\_dataset.py}, \texttt{data/corpora.py} \\
\cref{sec:axioms} & \texttt{constraints/} & \texttt{axioms/admissibility.py} \\
\cref{sec:stage-admiss,sec:certificates} & \texttt{constraints/} & \texttt{axioms/stage\_conditions.py}, \texttt{certificates/} \\
\cref{sec:guarantee} & \texttt{constraints/} & \texttt{guarantee/invariants.py} \\
\cref{sec:boosting}, Alg.~\ref{alg:boosting-bb} & \texttt{search/} & \texttt{baselines/}, \texttt{residuals/}, \texttt{expression/}, \texttt{boosting/}, \texttt{hard\_path/} \\
\cref{sec:soft} & \texttt{search/} & \texttt{soft\_path/} \\
\cref{sec:models} & \texttt{modeling/} & \texttt{models/} \\
\cref{sec:testbeds} (Neocortex) & \texttt{modeling/} & \texttt{training/} \\
\cref{sec:testbeds,sec:baselines} (AMA27/CPU) & \texttt{systems/} & \texttt{hpc/} \\
\cref{sec:deliverables} & \texttt{systems/} & \texttt{pipeline/run\_discovery.py} \\
\end{longtable}
}
