...
Two computational bottlenecks motivate ByteBoost participation, and each maps cleanly onto a different testbed. Pretraining the scaling grid is throughput-bound across hundreds of short runs; Neocortex at PSC (Cerebras CS-3 wafer-scale systems) is the natural match for the larger models in our $N$ range, where a single WSE-3 can keep the model on-chip without multi-GPU model parallelism. Comparing the resulting loss curves against our existing NVIDIA GPU baselines (an allocation of 200 NVIDIA GH200 superchips on DeltaAI at NCSA, plus a local four-GH200 cluster) is the modeling track. The symbolic regression search is a separate, CPU-bound bottleneck, highly parallel across genetic-programming populations and dominated by interval-arithmetic certification of candidates; that workload is largely scalar / imperfectly vectorized and does not require A64FX-style SVE or HBM. Stony Brook's AMA27 AmpereOne cluster (ACCESS: phase~1 has 312 single-socket nodes with 192-core A192-32M CPUs and 384\,GB DDR5 each) supplies Arm ISA diversity against our three 256-core AMD EPYC (x86) CPU baselines and dense core counts for concurrent candidate evaluation, the Stony Brook Arm platform featured in ByteBoost~2.0 after retirement of Ookami.
